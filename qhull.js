...
(function() {
  var Face, FaceLink, INSTRUCTIONS, QuickhullSolver, SHOW_STATS, TRACE_HORIZON, TRACE_INITIAL, TRACE_MANY_HORIZON_PATHS, TRACE_NEW_FACES, TRACE_NEW_FACE_EACH, TRACE_TERMINATE, Tracer, animate, assignPoints, checkFace, checkHorizon, checkHull, init, isFace, isVector, lastKey, makePoints, onKeyDown, onMouseWheel, random, render, reset, seed;
  ...
  QuickhullSolver = (function() {
    ...
    QuickhullSolver.prototype.expandHull = function(face) {
      var d, horizon, newFaces, prev, visited, _ref, _ref1;
      ...
      if (TRACE_NEW_FACE_EACH) {
        newFaces.forEach(function(f) {
          return this.tracer.trace("newFace", {
            faces: [f],
            vertices: f.assignedPoints,
            hiVertices: f.furthest.point ? [f.furthest.point] : []
          });
        });
      }
      if (TRACE_NEW_FACES) {
      ...
      this.tracer.trace("new hull", {
        faces: this.hull,
        vertices: points,
        subtitle: "" + (_.filter(this.hull, function(h) {
          return !h.deleted;
        }).length) + " faces"
      });
      this.hull.forEach(checkFace);
